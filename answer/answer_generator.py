"""
Antwoord generator — ondersteunt Anthropic, Google Gemini en Ollama.
"""
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "Je bent een assistent voor cultureel erfgoed. "
                    "Geef korte, duidelijke Nederlandse antwoorden."
                ),
            },
            {
                "role": "user",
...
def generate(question: str, results: dict[str, Any]) -> str:
    ...
    prompt = (
        ...
        f"{summary}\n\n"
        "Geef een beknopt, informatief antwoord in het Nederlands op de originele vraag. "
        f"Als de vraag om een telling gaat, gebruik dan het totaal aantal rijen ({total}) als antwoord. "
        "Gebruik de data. Geen technische termen. Maximaal 3-4 zinnen."
    )

    logger.info(
        "Antwoord genereren via %s voor %d resultaten",
        config.LLM_PROVIDER,
        total,
    )
